...
let practiceSession = null; // {id, createdAt, mode, filters, questionIds, seqMap, index, answeredMap, correctCount}
...
modeSelect.addEventListener('change', () => {
  ...
  if (currentMode === 'chapter') {
    if (chSel) { chSel.multiple = true; chSel.size = Math.min(8, (chSel.options||[]).length || 6); }
  } else {
    if (chSel) { chSel.multiple = false; chSel.size = 1; }
  }

  // Start fresh session on mode change
...
// Practice sessions controls
const btnNewSession = document.getElementById('btnNewSession');
if (btnNewSession) btnNewSession.addEventListener('click', () => startNewPracticeSession(true));
const btnSessions = document.getElementById('btnSessions');
if (btnSessions) btnSessions.addEventListener('click', showSessionsModal);
const btnCloseSessions = document.getElementById('btnCloseSessions');
if (btnCloseSessions) btnCloseSessions.addEventListener('click', () => document.getElementById('sessionsModal')?.classList.add('hidden'));
...
async function buildPracticePool() {
  ...
  if (currentMode === 'due') {
    filtered = filtered.filter(q => isDue(q, nowIso));
    if (!filtered.length) filtered = all.filter(q => q && q.active !== false);
  } else if (currentMode === 'new') {
    filtered = filtered.filter(q => !q.timesSeen);
  } else if (currentMode === 'wrong') {
    ...
    filtered = filtered.filter(q => (q.timesWrong || 0) > 0 || (q.easeFactor || 2.5) < 2.2);
  } else if (currentMode === 'chapter') {
    // multi-select support or comma-separated filter
    ...
    if (!chapters.length) {
      const raw = (document.getElementById('chapterFilter')?.value || '').trim();
      if (raw) {
        chapters = raw.split(',').map(s => s.trim()).filter(Boolean);
      }
    }
    if (chapters.length) {
  ...
  // Sort deterministic: dueAt then least seen
  filtered.sort((a,b) => {
    const ad = a.dueAt || '';
    const bd = b.dueAt || '';
    if (ad && bd && ad !== bd) return ad.localeCompare(bd);
...
async function startNewPracticeSession(autoStartFirst=true) {
  ...
  practiceSession = {
    ...
    filters: {
      mode: currentMode,
      chapters: ids.map(id => (picked.find(q=>q.id===id)?.chapter || '')).filter(Boolean),
      prefSkipSolved: !!prefSkipSolved
    },
    questionIds: ids,
    seqMap,
    index: 0,
    answeredMap: {}, // {qid:{selectedIndex,isCorrect,answeredAt}}
    correctCount: 0
  };
...
function renderSequenceNav() {
  ...
  ids.forEach((id, idx) => {
    ...
    if (ans) {
      cls += ' answered ' + (ans.isCorrect ? 'correct' : 'wrong');
    }
    html += `<button class="${cls}" data-seq="${idx}">${seq}</button>`;
  });
  nav.innerHTML = html;
...
async function jumpToSessionIndex(idx) {
  ...
  // restore previous answer UI state
...
async function resumePracticeSession(id) {
  ...
  // set currentMode to session mode for UI consistency
  currentMode = practiceSession.mode;
  const modeSel=document.getElementById('modeSelect');
  if (modeSel) modeSel.value=currentMode;

  // move to first unanswered if any
...
async function loadNextQuestion(resetHistory) {
  ...
  if (resetHistory) {
    ...
    practiceSession.index = Math.min(practiceSession.index + 1, practiceSession.questionIds.length - 1);
  }
  ...
  tx.oncomplete = async () => {
    ...
    if (practiceSession && practiceSession.questionIds?.length) {
      ...
      if (!prev) {
        ...
        // allow changing answer without double-counting correct
      ...
      const sessions = await loadPracticeSessions();
      const idx = sessions.findIndex(s => s.id === practiceSession.id);
      if (idx >= 0) {
        sessions[idx].answeredMap = practiceSession.answeredMap;
        sessions[idx].answered = Object.keys(practiceSession.answeredMap).length;
        sessions[idx].correct = practiceSession.correctCount;
...
async function toggleFlag() {
  ...
  tx.oncomplete = async () => {
    ...
    if (practiceSession && practiceSession.questionIds?.length) {
      ...
      const prev = practiceSession.answeredMap[qid];
      if (!prev) {
        // first time answered in this session
        practiceSession.answeredMap[qid] = { selectedIndex: selectedIdx, isCorrect, answeredAt: now };
        if (isCorrect) practiceSession.correctCount += 1;
        // persist session summary update
        const sessions = await loadPracticeSessions();
        const s = sessions.find(x=>x.id===practiceSession.id);
        if (s) {
          s.answeredMap = practiceSession.answeredMap;
          s.answered = Object.keys(practiceSession.answeredMap).length;
          s.correct = practiceSession.correctCount;
          await savePracticeSessions(sessions);
        }
      } else {
        // overwrite selection but don't double count
        prev.selectedIndex = selectedIdx; prev.isCorrect = isCorrect; prev.answeredAt = now;
      }
      renderSequenceNav();
    }
    renderQuestion();
